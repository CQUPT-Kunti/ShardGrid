...
def run_rtx4060_remote_access_check(
...
def test_remote_access_success_path_uses_ssh_transport(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    responses = [
        ...
        _result(stdout="/home/shardgrid/miniconda3/bin/conda"),
        _result(stdout="none|none"),
        _result(
            stdout=(
                "# conda environments:\n"
                "base                  *  /home/shardgrid/miniconda3\n"
                "shardgrid                /home/shardgrid/miniconda3/envs/shardgrid\n"
            )
        ),
        _result(stdout="Python 3.12.13\n"),
        _result(stdout="/home/shardgrid/miniconda3/envs/shardgrid/bin/python\n"),
    ]

    def fake_run(command: list[str] | tuple[str, ...] | str) -> ProcessResult:
        calls.append(command if isinstance(command, str) else " ".join(command))
        return responses.pop(0)

    monkeypatch.setattr(transport, "run", fake_run)

    outcome = run_rtx4060_remote_access_check(transport, worker)

    assert isinstance(outcome.transport, SSHTransport)
    assert outcome.status == "PASS"
    assert outcome.windows_identity == "LDJ"
...
@pytest.mark.parametrize(
    ...
    [
        (
            "ssh: connect to host 10.87.5.155 port 22: No route to host",
            False,
            "host_unreachable",
        ),
        (
            "ssh: connect to host 10.87.5.155 port 22: Connection timed out",
            True,
            "connection_timeout",
        ),
        (
            "shardgrid@10.87.5.155: Permission denied "
...
def test_remote_access_classifies_connection_failures(
    ...
    def fake_run(command: list[str] | tuple[str, ...]) -> ProcessResult:
...
def test_remote_access_rejects_python_outside_selected_conda(
    ...
    responses = [
        ...
        _result(stdout="Python 3.12.13\n"),
        _result(stdout="/usr/bin/python3\n"),
    ]
...
def test_live_remote_access_attempt_to_rtx4060_records_real_result() -> None:
    config, worker = _load_live_worker_config()
    transport = _build_transport(config, worker)
    ...
    assert outcome.host == "10.87.5.155"
