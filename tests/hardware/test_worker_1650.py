"""Real GTX 1650 single-GPU CUDA/PyTorch smoke test (T035).

This test is designed to run INSIDE the WSL2 training runtime of the GTX 1650
...
The test verifies the actual detected GPU is the target GTX 1650 and executes
a real CUDA tensor operation.  A JSON hardware finding (PASS / FAIL / BLOCKED
/ PENDING) is always written with the real evidence; it never infers success
from configuration, worker name, or mock data.
"""
...
EXPECTED_GPU_SUBSTRING = "GTX 1650"
TARGET_WORKER_ID = "gpu1650"
TARGET_HOST = "10.87.5.15"
TARGET_HOSTNAME = "LAPTOP-5G3QUOGM"
TENSOR_SIZE = 1024
...
def _write_finding(payload: dict[str, Any]) -> Path:
    ...
    timestamped = directory / f"gtx1650-smoke-{_now()}.json"
    timestamped.write_text(json.dumps(payload, indent=2, sort_keys=True))
    (directory / "gtx1650-latest.json").write_text(
...
def test_gtx1650_single_gpu_cuda_smoke() -> None:
    ...
    finding["status"] = "PASS"
    path = _write_finding(finding)
    print(f"GTX 1650 hardware smoke PASS; finding: {path}")